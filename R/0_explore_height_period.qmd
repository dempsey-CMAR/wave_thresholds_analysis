...
- Teledyne has recommended that we do not publish zero-up crossing parameters (`average_height_largest_10_percent_m`, `average_height_largest_33_percent_m`, `maximum_height_m`, and the corresponding period measurements) from 4 beam ADCPs.
  - The zero-up crossing parameters rely on the use of acoustic surface tracking.
  - The 4 beams can often "skip away" from the ADCP and provide a poor surface tracking measurement.
  - See email from Paul Devine (October 2024)

These figures show the value of all variables when the highlighted variable is less than or equal to zero. For example, in the tab `significant_height_m`, the data is filtered to observations where `significant_height_m <= 0`.
